...
def load_csv(uploaded_file) -> pd.DataFrame:
    """Read uploaded file — try UTF-8 first, fall back to latin-1."""
    try:
...
def df_to_csv_bytes(df: pd.DataFrame) -> bytes:
    """
    Export with UTF-8 BOM (utf-8-sig) so Excel opens accented characters
    like SOCIÉTÉ GÉNÉRALE correctly without manual encoding selection.
    """
    buf = io.StringIO()
...
def format_iso_set(iso_set: set, label_map: dict) -> str:
    """Turn a set of ISO codes into 'FR (FRANCE), LU (LUXEMBOURG)' style string."""
    if not iso_set:
        return "None"
    parts = sorted(f"{iso} ({label_map.get(iso, '?')})" for iso in iso_set)
    return ", ".join(parts)
...
# ── Country label lookup (DKL + referential) ─
country_label_map = (
...
for i, entity_id in enumerate(entities_both_list):
    ...
    for iso in actual_isos:
        ...
        if le_detail.empty:
            le_rows_iter = [{"id_legal_entity": None, "full_name": "NOT FOUND IN REFERENTIAL", "elr_code": None, "expected_iso": None}]
        else:
            le_rows_iter = le_detail.to_dict("records")
        ...
        for le_row in le_rows_iter:
            ...
            base = {
                "id_entity":         entity_id,
                "id_legal_entity":   int(le_id) if le_id is not None and not pd.isna(le_id) else None,
            ...
            if is_valid:
                ...
                # Three flavours of mismatch note
...
# Scenario 1: only entities where NO mismatch exists at all
...
SCENARIO_DESCRIPTIONS = {
    "tab1": (
        "**Scenario 1 — All Valid:** Datasets where every assigned country matches the "
        "expected country from the legal entity referential. No migration action needed."
    ),
    "tab2": (
        "**Scenario 2 — Country Mismatch:** Datasets that have both a legal entity and a country "
        "but at least one country does NOT match the referential mapping. "
        "The `scenario_note` column shows exactly: *'Dataset is mapped to X, but legal entity Y should map to Z per referential'*."
    ),
    "tab3": (
        "**Scenario 3 — Legal Entity, No Country:** Datasets that have a legal entity linked "
        "but zero countries assigned. The note shows what country should be added per referential."
    ),
    "tab4": (
...
st.caption("All CSVs use UTF-8 BOM encoding — accented characters (é, è, à …) display correctly when opened in Excel.")
